...
import { basketOutline, chevronDown, locateOutline, notificationsOutline } from 'ionicons/icons';
...
const Home: React.FC = (props: any) => {
  ...
  useEffect(() => {
    axios.get('/restaurants/nearby')
      .then(res => {
        const data = res.data;
        setRestaurants(data.restaurants);
        setCategories(data.categories);
        const featured = data.restaurants.filter((restaurant) => restaurant.featured === 1)
      ...
      <IonHeader className="bg-white ion-no-border border-bottom">
        <IonToolbar>
          <IonButtons slot="primary">
            <IonButton onClick={() => {setHasNotification(false); props.history.push('/notifications')}} className="position-relative">
          ...
          <IonTitle size="small" className="font-weight-bold text-default p-0">
            <Link to="/address">
              <small className="m-0 font-weight-bold text-uppercase text-danger">Deliver at</small>
              <IonIcon icon={chevronDown} className="align-text-top text-dark ml-1" />
            </Link>
          </IonTitle>
          <IonTitle size="small" className="font-weight-bold text-default p-0">
            <Link to="/address">
              <small className="nav-address text-default">{default_address && default_address.full_address}</small>
            </Link>
          </IonTitle>
        </IonToolbar>
      ...
      <IonContent fullscreen className="bg-white">
        ...
        <Container fluid={true} className="my-3 p-0">
          ...
          <Row className="my-2">
            <Col lg="5">
              {load ? (
                (featured.length > 0) && <FeaturedRestaurant history={props.history} restaurants={featured} />
        ...
        <Container fluid={true} className="mt-3">
          ...
          <Row>
            <Col lg="5">
              {load ? (
                (restaurants.length > 0) && <RestaurantList history={props.history} restaurants={restaurants} />
              ) : <GeneralSkeletonText />}
            </Col>
          </Row>
        </Container>
